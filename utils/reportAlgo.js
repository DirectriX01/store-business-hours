...
const self = (module.exports = {
  async reportGeneration(storeId) {
    const p1 = StoreTimezone.find({ store_id: storeId }).exec();
    const p2 = StoreHours.find({ store_id: storeId }).exec();
    const p3 = StoreReport.findOne({ store_id: storeId }).exec();
    const [storeHours, storeTimezone, storeReport] = await Promise.all([p1, p2, p3]);

    if (Object.keys(storeTimezone).length === 0 || !storeTimezone.timezone_str) {
      storeTimezone.timezone_str = 'America/Chicago';
    }
    if (!storeReport) {
    ...
    const lastReportTime = moment(storeReport.last_updated).tz(storeTimezone.timezone_str);
    const now = moment().tz(storeTimezone.timezone_str);
    ...
    const storeHoursArr = new Array(6).fill([
      [
        {
          start_local_time: '00:00:00',
          end_local_time: '23:59:59',
        },
      ],
    ]);

    for (let i = 0; i < storeHours.length; i += 1) {
      storeHoursArr[storeHours[i].day_of_week].push(storeHours[i]);
    }

    storeHoursArr.forEach((arr) => {
      arr.sort((a, b) => a.start_local_time.localeCompare(b.start_local_time));
    });
  ...
  async reportGenerationAll(storeIds) {
